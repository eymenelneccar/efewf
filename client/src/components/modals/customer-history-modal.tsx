...
export default function CustomerHistoryModal({
  ...
  const totalSpent = customerTransactions
    .filter(t => (t as any).transactionType !== "debt_collection")
    .reduce((sum, t) => sum + Number(t.total || 0), 0);

  const completedTransactions = customerTransactions.filter(
    (t) => t.status === "completed" && (t as any).transactionType !== "debt_collection"
  );

  const pendingTransactions = customerTransactions.filter(
    (t) => t.status === "pending" && (t as any).transactionType !== "debt_collection"
  );
  ...
  const printCustomerHistory = (customer: Customer, transactions: Transaction[], totalSpent: number) => {
    ...
    const completedTransactions = transactions.filter(t => t.status === "completed" && (t as any).transactionType !== "debt_collection");
    const pendingTransactions = transactions.filter(t => t.status === "pending" && (t as any).transactionType !== "debt_collection");
