...
export default function TransactionsTable({ currency = "₺" }: Props) {
  ...
  const debtCollectionTransactions = transactions.filter(t => 
    (t as any).transactionType === 'debt_collection'
  );
  ...
  const paidTransactions = regularTransactions.filter(t => 
    t.paymentType === 'cash' || t.status === 'completed'
  );

  const unpaidTransactions = regularTransactions.filter(t => 
    t.paymentType === 'credit' && t.status !== 'completed'
  );
  ...
  const getStatusText = (status: string, paymentType?: string, transactionType?: string) => {
    ...
    switch (status) {
      case "completed":
        return paymentType === 'credit' ? "مبيعات آجل" : "مكتملة";
      case "pending":
        return "معلقة";
      case "cancelled":
        return "ملغية";
  ...
  // عرض المعاملات بالترتيب: غير المسددة، ثم تحصيلات الديون، ثم المسددة
  const allTransactionsOrdered = [...unpaidTransactions, ...debtCollectionTransactions, ...paidTransactions];
  const totalPages = Math.ceil(allTransactionsOrdered.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentTransactions = allTransactionsOrdered.slice(startIndex, endIndex);
            ...
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-600 font-medium">
                  مجموع المبيعات: {regularTransactions.length}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-red-500 rounded-full"></div>
                <span className="text-red-600 font-medium">
                  ديون معلقة: {unpaidTransactions.length}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-blue-500 rounded-full"></div>
                <span className="text-blue-600 font-medium">
                  تحصيلات: {debtCollectionTransactions.length}
                </span>
              </div>
            </div>
                  ...
                  {currentTransactions.map((transaction: Transaction) => (
                    <tr key={transaction.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <Badge className={getStatusColor(transaction.status ?? "pending", (transaction as any).transactionType)}>
                          {getStatusText(transaction.status ?? "pending", transaction.paymentType ?? undefined, (transaction as any).transactionType)}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-600">
            ...
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between">
                <div className="text-sm text-slate-600">
                  عرض {startIndex + 1}-{Math.min(endIndex, allTransactionsOrdered.length)} من أصل {allTransactionsOrdered.length} معاملة
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
