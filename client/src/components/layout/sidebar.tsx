    ...
    <div className="bg-white shadow-lg w-64 flex-shrink-0 flex flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">
              {localStorage.getItem('companyName') || 'ERP Pro'}
            </h1>
            <p className="text-sm text-slate-500">
              {localStorage.getItem('companySubname') || 'نظام إدارة متطور'}
            </p>
          </div>
        </div>
        <div className="mt-3 pt-3 border-t border-slate-100">
          <p className="text-xs text-slate-400 text-center">
            صُنع بواسطة ايمن © {new Date().getFullYear()}
          </p>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-200">
        <div className="flex items-center gap-3 mb-3">
          <img 
            src={localStorage.getItem('userProfileImage') || (user as any)?.profileImageUrl || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-4.0.3&auto=format&fit=crop&w=150&h=150"} 
            alt="صورة المستخدم" 
            ...
            <p className="text-sm font-medium text-slate-800 truncate">
              {localStorage.getItem('userFullName') || 
               ((user as any)?.firstName && (user as any)?.lastName 
                ? `${(user as any).firstName} ${(user as any).lastName}`
                : (user as any)?.username || "Eymen Admin")}
            </p>
